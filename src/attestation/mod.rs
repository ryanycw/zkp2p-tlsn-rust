...
pub async fn notarize_transcript<T>(
    prover: &mut Prover<T>,
    transcript_commit: TranscriptCommitConfig,
    example_type: &ExampleType,
) -> Result<(tlsn_core::attestation::Attestation, tlsn_prover::SessionHeader), Box<dyn std::error::Error>> {
...
pub async fn save_attestation_files(
    ...
    secrets: &tlsn_prover::SessionHeader,
    example_type: &ExampleType,
) -> Result<(), Box<dyn std::error::Error>> {
