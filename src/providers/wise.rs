...
pub async fn execute_dual_phase_requests(
    ...
    if phase1_response.status() != StatusCode::OK {
        return Err(format!(
            "❌ Phase 1 failed - Transaction list request returned: {}",
            phase1_response.status()
        )
        .into());
    }
    ...
    println!(
        "   ✅ Transaction list retrieved ({} bytes)",
        phase1_body.len()
    );
    println!("{}", phase1_text);
    ...
    let transaction_exists =
        crate::verify_transaction_in_list(&transaction_list, &wise_config.transaction_id)?;

    if !transaction_exists {
        return Err(format!(
            "❌ Transaction {} not found in user's transaction list - cannot prove ownership",
    ...
    println!(
        "   ✅ Transaction ownership verified: {} found in user's list",
        wise_config.transaction_id
    );
    ...
    if phase2_response.status() != StatusCode::OK {
        return Err(format!(
            "❌ Phase 2 failed - Transaction details request returned: {}",
            phase2_response.status()
        )
        .into());
    }
