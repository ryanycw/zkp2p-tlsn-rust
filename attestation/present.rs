...
async fn create_presentation(example_type: &ExampleType) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "🎭 Creating selective disclosure presentation for {:?}...",
        example_type
    );
    ...
    match example_type {
        ExampleType::WiseTransaction => {
            ...
            if transcript.requests.len() < 2 || transcript.responses.len() < 2 {
                return Err(
                    "Dual-phase attestation should contain at least 2 requests and 2 responses"
                        .into(),
                );
            }
        }
    ...
    match example_type {
        ExampleType::WiseTransaction => {
            ...
            for header in &list_request.headers {
                ...
                let should_hide_value = header_name
            ...
            if let tlsn_formats::http::BodyContent::Json(list_json) = list_content {
                ...
                if let Some(data) = list_json.get("data") {
                    let target_transaction_id =
                        std::env::var("WISE_TRANSACTION_ID").unwrap_or_default();
                    println!(
                        "     Searching for target transaction ID: {}",
                            ...
                            if found_target {
                                ...
                                if let Some(transaction_obj) = data.get(&index.to_string()) {
                                    if let Some(id_field) = transaction_obj.get("id") {
                                        ...
                                        println!(
                                            "     Revealed: Target transaction ID {} for ownership proof",
                                            target_transaction_id
                                        );
                                    }
                                    if let Some(resource) = transaction_obj.get("resource") {
                                        if let Some(id_field) = resource.get("id") {
                                            builder.reveal_recv(id_field)?;
                                            println!(
                                                "     Revealed: Target transaction resource.id {} for ownership proof",
                                                target_transaction_id
                                            );
                                        }
                                    }
    ...
    match example_type {
        ...
        _ => {
            ...
            println!(
                "📁 The presentation has been written to: {}",
                presentation_path
            );
        }
    }
    ...
    println!(
        ...
        example_type.to_string().to_ascii_lowercase()
    );
